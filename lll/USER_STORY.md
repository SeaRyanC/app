...
- As a coach, there is no "Off" eligibility checkbox — all players can always bench. Instead, checking "Off+" marks a player as bench-preferred: they are preferentially given bench slots over non-Off+ players when bench slots are being distributed. Equal overall play time is still maintained for all players (Off+ players fill the naturally-occurring "extra" bench slots, keeping max − min Off innings ≤ 1 across everyone).
...
- As a coach, the generated lineup satisfies all hard criteria:
  ...
  - **Position rotation is enforced as a hard constraint for all infield positions (P, C, 1B, 2B, 3B, SS)**: a player is never assigned an infield position when there are other eligible, unassigned players who have not yet played that position this game. Only when every eligible candidate has already played the position at least once are repeats allowed. This ensures that with 7 eligible pitchers and 5 innings, no pitcher ever repeats. This rule does not apply to OF or Off.
  ...
  - Players only bench when all field positions (infield + OF) are already filled for that inning — no one sits out unnecessarily
  - **Among players satisfying the rotation constraint, balanced bench time is the primary tiebreaker when assigning infield slots**: the algorithm prefers players who have accumulated the most "Off" innings so far — those who have been benched the most get the next available field slot. Among players tied on Off count, Off+ players are preferred to remain on the bench (non-Off+ players fill field slots first when Off counts are equal), then per-position round-robin applies, and "+" priority is the final tiebreaker. For OF slots, the same ordering applies.
- As a coach, the generated lineup tries to satisfy soft criteria to pick the best schedule among all candidates, evaluated in priority order:
  1. **Balanced Off innings** — schedules where every player's Off-inning count differs by at most 1 beat all schedules where the Off counts are more uneven. Off+ players naturally occupy the "extra" bench slots but still stay within this window (e.g. Off+ get 2 bench, others get 1, so max − min = 1). This is the highest-priority soft criterion.
