...
- As a coach, the generated lineup satisfies all hard criteria:
  ...
  - **Balanced bench time is the primary goal when assigning field slots**: when picking a player for any infield or OF slot, the algorithm first prefers players who have accumulated the most "Off" innings so far — those who have been benched the most get the next available field slot. Among players tied on Off count, per-position round-robin applies as a secondary criterion, and "+" priority is the tiebreaker. This ensures bench time stays within 1 inning across all players, even at the expense of per-position rotation balance.
