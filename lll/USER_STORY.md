...
- As a coach, the generated lineup satisfies all hard criteria:
  ...
  - **Among players satisfying the rotation constraint, balanced bench time is the primary tiebreaker when assigning infield slots**: the algorithm prefers players who have accumulated the most "Off" innings so far — those who have been benched the most get the next available field slot. Among players tied on Off count, per-position round-robin applies as a secondary criterion, and "+" priority is the tiebreaker. For OF slots, balanced bench time (most Off) remains the primary criterion since OF rotation is not enforced.
